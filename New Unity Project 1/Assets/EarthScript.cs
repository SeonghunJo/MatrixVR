...
public class EarthScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            //확대되는 효과
            //Camera.main.orthographicSize -=  5 * Time.deltaTime;       //zoom in
            
            //if (Camera.main.orthographicSize <= 1.0f)
               // Application.LoadLevel(1);   //scene 전환
    ...
    void OnMouseDown()
    {
        ...
        if(rotatePosY >=20 && rotatePosY <= 60 && clickPos.x >=200 && clickPos.x <= 320 && clickPos.y >= 70 && clickPos.y <= 230) //남아메리카
        {
            leftClick = true;
            chooseWorld = 2;
        }
